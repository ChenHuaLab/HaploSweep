...
void *calloc_align_ptr(int64_t size, int8_t align_bytes){
	...
	void *mem_ptr = (void *)(
                          ((int64_t)(base_ptr+align_bytes-1))
                          &~
                          ((int64_t)(align_bytes-1))
                          );
	if(mem_ptr==base_ptr) mem_ptr=base_ptr+align_bytes;
	*((int8_t *)mem_ptr-1)= (int8_t)(mem_ptr-base_ptr);
	return mem_ptr;
}

void free_align_ptr(void *ptr){
	free((void *)(ptr-*((int8_t *)ptr-1)));
}
...
void gz_stream_destory(GzStream *gz){
    if(gz->is_write){
        ...
        if(gz->bz2File) BZ2_bzReadClose(&(gz->bzerror), gz->bz2File);
        else gzclose(gz->fp);
    }
...
int bam_header(int l_chrNames, char **chrNames, int *chrLengths, char *ID, char *SM, char *LB, char *buff){
    int i;
...
void skip_bam_header(gzFile bam, char *buff){
    int32_t i, datasize, n_targets, name_len, target_len;
    gzread(bam, buff, 4);
    gzread(bam, &datasize, 4);
    gzread(bam, buff, datasize);
    gzread(bam, &n_targets, 4);
    for(i=0;i<n_targets;i++){
        gzread(bam, &name_len, 4);
        gzread(bam, buff, name_len);
        gzread(bam, &target_len, 4);
    }
}
...
long *copy_long_array(int size, long *array){
    long *res=my_new2(size*sizeof(long));
    memcpy(res, array, size*sizeof(long));
    return res;
}
...
void mysort_long(long *a, int left, int length){
        int right=left+length-1, i, j, k, m, n;
        ...
        long *work=my_new2(blen*sizeof(long));
        int workBase=0;
...
void mysort_long_inner(long *a, int left, int right, int leftmost){
...
void mysort_longlong(long long *a, int left, int length){
        int right=left+length-1, i, j, k, m, n;
        ...
        // Use or create temporary array b for merging
        long long *b;                 // temp array; alternates with a
        int ao, bo;              // array offsets from 'left'
        int blen = right - left; // space needed for b

        long long *work=my_new2(blen*sizeof(long long));
        int workBase=0;
...
void mysort_longlong_inner(long long *a, int left, int right, int leftmost){
        int length = right - left + 1, i, j, k, m, n;
        ...
        // Sort these elements using insertion sort
        if (a[e2] < a[e1]) { long long t = a[e2]; a[e2] = a[e1]; a[e1] = t; }

        if (a[e3] < a[e2]) { long long t = a[e3]; a[e3] = a[e2]; a[e2] = t;
            if (t < a[e1]) { a[e2] = a[e1]; a[e1] = t; }
        }
        if (a[e4] < a[e3]) { long long t = a[e4]; a[e4] = a[e3]; a[e3] = t;
            if (t < a[e2]) { a[e3] = a[e2]; a[e2] = t;
                if (t < a[e1]) { a[e2] = a[e1]; a[e1] = t; }
            }
        }
        if (a[e5] < a[e4]) { long long t = a[e5]; a[e5] = a[e4]; a[e4] = t;
            if (t < a[e3]) { a[e4] = a[e3]; a[e3] = t;
                if (t < a[e2]) { a[e3] = a[e2]; a[e2] = t;
                    if (t < a[e1]) { a[e2] = a[e1]; a[e1] = t; }
                }
        ...
        if (a[e1] != a[e2] && a[e2] != a[e3] && a[e3] != a[e4] && a[e4] != a[e5]) {
             ...
             * second terciles of the array. Note that pivot1 <= pivot2.
             */
            long long pivot1 = a[e2];
            long long pivot2 = a[e4];
...
void mysort_void(void **arr, int left, int length, int (*mycompare)(void *a, void *b)){
        ...
        void **work=my_new2(blen*sizeof(long));
        int workBase=0;
