...
#define my_new(num,size) ({char *res=calloc(num, size);if(!res){fprintf(stderr, "[%s:%s:%d]:calloc memory error!\n",__FILE__,__func__,__LINE__);exit(0);}res;})
#define my_new2(size) ({char *res=malloc(size);if(!res){fprintf(stderr, "[%s:%s:%d]:malloc memory error!\n",__FILE__,__func__,__LINE__);exit(0);}res;})
#define my_renew(ptr,size) ({char *res=realloc(ptr,size);if(!res){fprintf(stderr, "[%s:%s:%d]:realloc memory error!\n",__FILE__,__func__,__LINE__);exit(0);}res;})
...
void mysort_long_inner(long *a, int left, int rigth, int leftmost);
...
double chi2_cdf_upper(double x, int k); //equals: gsl_cdf_chisq_Q in(#include <gsl/gsl_cdf.h>) (gcc lib: -lgsl -lgslcblas)
//================ socket
