...
static int32_t hash_long(long key){
...
Entry_ld *hash_ld_get(Hash_ld *hp, long key){
...
void hash_ld_put(Hash_ld *hp, long key, double value){
    int32_t tmpHash=hash_long(key);
...
void hash_ld_remove(Hash_ld *hp, long key){
    int32_t tmpHash=hash_long(key);
    int32_t i=indexFor(tmpHash, hp->tableSize);
    Entry_ld *e=hp->table[i];
    Entry_ld *pre=NULL;
...
Entry_li *hash_li_get(Hash_li *hp, long key){
    int32_t tmpHash=hash_long(key);
...
void hash_li_put(Hash_li *hp, long key, int value){
    int32_t tmpHash=hash_long(key);
    int32_t i=indexFor(tmpHash, hp->tableSize);
...
void hash_li_remove(Hash_li *hp, long key){
    int32_t tmpHash=hash_long(key);
    int32_t i=indexFor(tmpHash, hp->tableSize);
    Entry_li *e=hp->table[i];
    Entry_li *pre=NULL;
...
Entry_ll *hash_ll_get(Hash_ll *hp, long key){
    int32_t tmpHash=hash_long(key);
...
void hash_ll_put(Hash_ll *hp, long key, long value){
    int32_t tmpHash=hash_long(key);
    int32_t i=indexFor(tmpHash, hp->tableSize);
...
void hash_ll_remove(Hash_ll *hp, long key){
    int32_t tmpHash=hash_long(key);
    int32_t i=indexFor(tmpHash, hp->tableSize);
    Entry_ll *e=hp->table[i];
    Entry_ll *pre=NULL;
...
AList_l *new_alist_l(int64_t initialCapacity){
    ...
    res->tableSize=initialCapacity;
    res->size=0;
    res->elementData=(long *)calloc(initialCapacity, sizeof(long));
    return res;
}
...
AList_l *alist_l_deep_copy(AList_l *list){
    ...
    res->tableSize=list->tableSize;
    res->size=list->size;
    res->elementData=(long *)malloc(list->tableSize*sizeof(long));
    memcpy(res->elementData, list->elementData, list->size*sizeof(long));
    return res;
}

void alist_l_add(AList_l *list, long value){
    long *pre=list->elementData;
...
static void alist_l_ensure(AList_l *list, int64_t minCapacity){
    if(minCapacity>list->tableSize){
        int64_t newCapacity=list->tableSize*3/2+1;
        list->tableSize=newCapacity;
        list->elementData=(long *)realloc(list->elementData, newCapacity*sizeof(long));
    }
}
...
BuffFile *new_buff_file(char *file, char *mode, int64_t write_file_size){
    BuffFile *bf=calloc(1, sizeof(BuffFile));
    bf->f=fopen(file, mode);
    if(bf->f==NULL){fprintf(stderr, "cannot open file(%s) with mode(%s)\n", file, mode);exit(0);}
    bf->tableSize=write_file_size;
    bf->size=0;
    bf->str=calloc(bf->tableSize, sizeof(char));
    return bf;
}

void buff_file_write_with_len(BuffFile *bf, char *str, int len){
    if((bf->size+len+1)>bf->tableSize){
        fprintf(bf->f, "%s%s", bf->str, str);
        bf->size=0;
        bf->str[0]='\0';
        return;
    }
    memcpy(bf->str+bf->size, str, len*sizeof(char));
    bf->size+=len;
    bf->str[bf->size]='\0';
}

void buff_file_write(BuffFile *bf, char *str){
    buff_file_write_with_len(bf, str, strlen(str));
}

void close_buff_file(BuffFile *bf){
    if(bf->size>0) fprintf(bf->f, "%s", bf->str);
    fclose(bf->f);
    free(bf->str);
    free(bf);
}
...
void java_random_set_seed(int64_t *java_random_seed, int64_t seed){
    *(java_random_seed)=(seed^0x5DEECE66DL)&((1L << 48) - 1);
}

static int java_random_next(int64_t *java_random_seed, int bits){
        long oldseed, nextseed;;
        while(1){
            oldseed = *(java_random_seed);
            nextseed = (oldseed * 0x5DEECE66DL + 0xBL) & ((1L << 48) - 1);
            if(oldseed!=nextseed){
                *(java_random_seed)=nextseed;
                break;
            }
        }
        return (int)((unsigned long)nextseed >> (48 - bits));
}

int java_random_next_int(int64_t *java_random_seed, int bound){
        int r = java_random_next(java_random_seed, 31);
        int m = bound - 1;
        if ((bound & m) == 0)  // i.e., bound is a power of 2
            ...
            for (u = r;
                 u - (r = u % bound) + m < 0;
                 u = java_random_next(java_random_seed, 31))
                ;
        }
        return r;
}

double java_random_next_double(int64_t *java_random_seed){
    int n1=java_random_next(java_random_seed, 26);
    int n2=java_random_next(java_random_seed, 27);
    return (((long)(n1) << 27)+n2) * 0x1.0p-53;
}
...
Pair *split_file_size(char *file, int num){
    ...
    if(sp<1000000){
        ...
        for(i=0;i<(long)num;i++){
            ...
            res[i].left= i==0? 0L:res[i-1].right;
